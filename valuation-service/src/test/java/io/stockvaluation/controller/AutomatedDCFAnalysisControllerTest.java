...
class AutomatedDCFAnalysisControllerTest {
    ...
    void postValuation_msftContractIncludesDcfAssumptionsAndVersionMetadata() {
        ...
        when(valuationWorkflowService.getValuation(eq("MSFT"), any(FinancialDataInput.class)))
                .thenReturn(output);
    ...
    @Test
    void getValuationRoute_isNotExposed() throws Exception {
        MockMvc mockMvc = MockMvcBuilders.standaloneSetup(controller).build();

        mockMvc.perform(get("/api/v1/automated-dcf-analysis/MSFT/valuation"))
