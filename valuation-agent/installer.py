...
class AgentInstaller:
    ...
    def _mcp_server_config(self) -> dict[str, object]:
        return {
            ...
            "args": ["-m", "valuation_agent.mcp_server"],
            "env": {
                "PYTHONPATH": str(PACKAGE_DIR),
                "STOCKVALUATION_SERVICE_URL": DEFAULT_SERVICE_URL,
            },
        }
    ...
    def _codex_mcp_block(self) -> str:
        ...
        escaped_pythonpath = _toml_string(str(PACKAGE_DIR))
        ...
        return "\n".join(
            [
                ...
                'args = ["-m", "valuation_agent.mcp_server"]',
                f'env = {{ PYTHONPATH = {escaped_pythonpath}, STOCKVALUATION_SERVICE_URL = {escaped_url} }}',
                CODEX_MCP_END,
                "",
            ]
        )
