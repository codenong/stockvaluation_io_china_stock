...
def test_install_skills_is_idempotent_for_codex_and_claude(tmp_path):
    ...
    assert "evidence-review-gate.md" in codex_skill.read_text(encoding="utf-8").lower()
    assert (codex_skill.parent / "references" / "mcp-tools.md").exists()
    assert (codex_skill.parent / "references" / "evidence-review-gate.md").exists()
...
def test_install_mcp_config_is_idempotent_for_claude_and_codex(tmp_path):
    ...
    assert claude_config["mcpServers"]["stockvaluation"]["args"] == [
        "-m",
        "valuation_agent.mcp_server",
    ]
    assert claude_config["mcpServers"]["stockvaluation"]["env"]["PYTHONPATH"] == str(PACKAGE_DIR)
    ...
    assert f'PYTHONPATH = "{PACKAGE_DIR}"' in contents
...
def test_check_env_reports_presence_without_printing_secret_values(tmp_path, capsys, monkeypatch):
    ...
    env_file.write_text(
        "POSTGRES_PASSWORD=postgres-secret\nDEFAULT_PASSWORD=default-live-secret\n",
        encoding="utf-8",
    )
    controller = ServiceController(
        project_dir=tmp_path,
    ...
    assert "DEFAULT_PASSWORD: set" in output
    assert output.count(": set") == 2
    assert "postgres-secret" not in output
    assert "default-live-secret" not in output
...
def test_check_env_detects_docker_compose_daemon_ports_and_placeholder_env(tmp_path):
    ...
    assert set(status.values) == {"POSTGRES_PASSWORD", "DEFAULT_PASSWORD"}
...
def test_status_reports_unreachable_when_health_probe_disconnects(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("valuation_agent.service_control.request.urlopen", disconnect)
    controller = ServiceController(
        project_dir=tmp_path,
        probe_runner=probe,
        docker_path_resolver=lambda _: "/usr/bin/docker",
