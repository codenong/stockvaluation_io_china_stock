...
- When a question carries `anchor_explanation`, show its source trail before the choices: the summary line, the segment rows (segment -> mapped industry, revenue weight, per-segment low/base/high), the weighted anchors, and any omitted-segment warnings. Use its exact source framing — segment-quantile anchors are "filing-based segment mix plus Damodaran industry quantiles", never simply "from the filing" — and present low/base/high as bounded anchors, never as recommendations. If warnings say the anchor is incomplete, say so.
- A question with `segment_scope` is a per-segment question: its A/B/C are that segment's own industry quantiles and its answer routes into `scenario.segments`. Structured custom D for these: `{"choice":"D","value":[{"segment":"<name>","field":"<driver>","value":<number>}]}`; never average per-segment values into one company number.
...
- A question with `requires_user_value: true` has no computable anchor: ask the user for a specific number and submit it through `stockvaluation.apply_guided_answers`, so the run records `source=user_input`. Never invent the number.
- After answers (or `use defaults`), call `stockvaluation.apply_guided_answers` with just the `run_id` and the answers — do not echo the plan back; the server uses its stored copy (`planSource: "run_state"`). For custom D answers, pass the numeric value as `{"choice":"D","value":15.5}` for that question id; do not turn it into prose. This records which anchor or user value actually mapped per driver and clears the guided gate. Then run exactly one final deterministic call: `stockvaluation.recalculate` with `request_policy.mode = "user_refined_scenario"` (ticker) or `stockvaluation.value_prospectus` with `prospectusScenarioCandidate.scenario` (prospectus).
...
2. Stop at the extraction review. Before this human gate, use the returned `prospectus.segmentReview` / `packet.segments` proposed mappings when available; the user should approve or correct proposed segment mappings, not start from blank segment judgment. Show a compact review card: what was extracted (company, filing metadata, offering price and basis, share counts, core financials, proposed segment mappings with confidence, score, rationale, row role, warnings, and raw segment candidate tables), what looks usable, what needs review, and a recommended action. Offer the allowed actions from `segmentReview.allowedActions`: approve mappings, correct a mapping, reject a mapping, leave a row unmapped, add sources, or stop. Recommend stop/correct for empty, ambiguous, or low-confidence material segment mappings. Approval is a modeling-input review, not financial advice.
