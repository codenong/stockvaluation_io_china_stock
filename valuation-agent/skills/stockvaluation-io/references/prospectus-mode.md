...
When using `stockvaluation.plan_guided_questions`, pass material prospectus segments as compact structured rows, not prose. Include `name`, `revenue_amount` or `revenue_2025` when weights are not disclosed, and reviewed mapping fields such as `sector_key`, `mapped_industry`, or `candidate_mapping`. After `stockvaluation.apply_guided_answers`, inspect `prospectusScenarioCandidate.scenario.segments`; if the user accepted a segment-modeling answer and this field is missing, stop and fix the planner input before calling `stockvaluation.value_prospectus`.
