...
9. Prospectus mode supports deterministic explicit scenarios through `stockvaluation.value_prospectus.scenario`. The scenario may include net proceeds, R&D capitalization, terminal return on capital, terminal cost of capital, terminal growth, company-level assumptions, and segment-level assumptions for Launch, Starlink / Connectivity, AI, and Other or expansion revenue. Do not call guided answers a user-refined scenario unless they were sent through this deterministic scenario path. If the visible guided flow says "Question 1 of 3" and the user accepts defaults, all remaining default answers must be summarized; do not skip hidden questions silently.
