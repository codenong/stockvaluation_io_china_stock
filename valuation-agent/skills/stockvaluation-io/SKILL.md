...
- After the evidence review gate is cleared and the evidence-constrained base case is built, build a hidden guided question plan that is materiality-driven. Use `stockvaluation.plan_guided_questions` when available, passing only compact baseline, evidence, plausibility, segment, and diagnostic context. Each planner evidence item must include `driver`, `evidence_summary` or `fact`, `source_url`, `source_date`, and `confidence`; for prospectus filing facts, repeat the SEC filing URL and filing date on each item. If the returned plan has `planner_warnings` or dropped evidence in `evidence_input_quality`, retry once with the dated/cited evidence packet before asking questions. Then ask every material company-specific question up to a hard cap of 15 visible guided questions. Ask one question at a time by default. Do not ask a batch of 4-6 questions unless the user explicitly requests batch mode.
...
14. Unless the user explicitly requested quick valuation, no questions, skip questions, one-shot report, automation, or smoke-test, run guided refinement using `{baseDir}/references/guided-valuation-refinement.md`. Build a hidden guided question plan of company-specific bounded questions after the evidence review gate is cleared, baseline plausibility is checked, and the evidence-constrained base exists; the plan must be materiality-driven. Use `stockvaluation.plan_guided_questions` when available. It plans questions only; it does not compute valuation math. Send dated and cited evidence items to the planner; if `planner_warnings` or `evidence_input_quality.dropped_evidence_item_count` is nonzero, retry once with complete source metadata before showing the user the plan. Ask every material company-specific question up to a hard cap of 15 visible guided questions, with no forced minimum. If only one or two questions matter, ask only those. If none matter, explain and do not invent filler. Ask one question at a time by default. Do not ask a batch of 4-6 questions unless the user explicitly requests batch mode. Do not treat a plain "value COMPANY using stockvaluation.io" request as a one-shot path.
...
4. Stop and ask the user to review the extracted company identity, filing metadata, `offering_price`, share-count basis, financial statement facts, raw segment candidate tables and rows, source table titles, and extraction issues. The review is educational modeling control and is not financial advice. Do not show the user only a bare list of allowed actions. Show a compact review card with what was extracted, what is missing or ambiguous, the recommended next action, and four numbered human choices: `1` approve and continue, `2` correct the packet, `3` add sources, or `4` stop. Map the number to the internal action names in `{baseDir}/references/prospectus-mode.md`; do not ask humans to type internal action names unless they are using automation.
...
6. Call `stockvaluation.value_prospectus` with the reviewed packet, and include a `scenario` object only when the user/model has explicit story assumptions. Use the returned `priceBasis = offering_price`; do not substitute Yahoo Finance, yfinance, market-data revenue estimates, or a live trading market price.
7. Read `valuationBasisStatus`, `valuationCaseStatus`, `proceedsBasis`, `valuationBasisWarnings`, `dcf.valueVisibility`, and `baseline.baselineUseStatus` immediately after `stockvaluation.value_prospectus`. If `valuationCaseStatus = challenged_valuation_case`, `dcf.valueVisibility = diagnostic_only`, or `baseline.baselineUseStatus = challenged_baseline`, say no clean user-facing valuation was produced. For `pro_forma_cash_missing`, say post-offering shares require pro-forma cash or net proceeds. For `gross_proceeds_estimate_only`, say only gross proceeds could be inferred and the basis is not clean. A clean cash/share basis does not make a challenged segment case clean. Do not show the internal diagnostic value before evidence review. If the user then chooses `continue with caveats`, asks for valuation detail, or asks for audit detail, show the returned `dcf.estimatedValuePerShare` as a clearly labeled challenged diagnostic value, not as intrinsic value, fair value, or a recommendation.
...
- Use `stockvaluation.plan_guided_questions` only after evidence review and baseline plausibility. It is a read-only planner for story-to-driver guided questions, not a valuation engine and not a substitute for `stockvaluation.recalculate`.
...
- Do not show `mechanical model value`, `mechanical baseline value`, raw `mechanical_only`, or other internal scaffolding values in the default user-facing report. If the service only produced a mechanical/challenged result, say no clean valuation case was produced and explain the blockers. If the user explicitly continued with caveats or asked for valuation/audit detail, show the returned diagnostic value as a challenged diagnostic value with the blockers next to it; do not call it a clean intrinsic value.
...
- When the baseline is challenged, mention baseline quality and unsupported blockers in the main narrative. Keep mechanical baseline value/detail out of the report unless the user explicitly asks for audit/debug detail. For prospectus challenged cases after `continue with caveats`, show only the returned prospectus diagnostic value, clearly labeled as challenged and diagnostic.
