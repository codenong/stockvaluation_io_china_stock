...
- Match CI/runtime toolchains when reproducing or validating work: Python `3.11` for `valuation-agent/` and `yfinance/`; Java `21` for `valuation-service/`.

## Service Map

- `valuation-agent/`: Python package for skill installation, service control, and the stdio MCP server.
- `valuation-agent/skills/stockvaluation-io/`: bundled Claude/Codex skill pack used by the user's agent to write educational valuation reports.
...
- If you change API shapes, auth headers, required env vars, valuation payload fields, or MCP tool contracts, verify every caller in `valuation-agent/`, `valuation-service/`, `yfinance/`, `docker-compose.local.yml`, and the bundled skill docs.
...
- Agent-native package changes: run `python3.11 -m pytest valuation-agent/tests/agent_native -q` and `./scripts/test_install.sh`.
...
- `.env.example` is the safe starting point for local env setup. Do not overwrite a user's existing `.env` without being asked.
